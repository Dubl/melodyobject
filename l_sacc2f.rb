...
client_name = CFStringCreateWithCString( nil, "MIDIator", 0 )
client1 = Pointer.new(:id) #DL::CPtr.new(DL::malloc(DL::TYPE_VOID) )
client2 = Pointer.new(:id) #DL::CPtr.new(DL::malloc(DL::TYPE_VOID) )
client3 = Pointer.new(:id) #DL::CPtr.new(DL::malloc(DL::TYPE_VOID) )
x=MIDIClientCreate( client_name, client1, client2, client3)
puts 'worked'
#combyn e: [2,3], f: [3,3], g: [2,4], h: [2,4], i: [2], j: [3,4], k: [4], l: [2], m: [3]


    yaml_file = File.expand_path('/Users/Jon/Desktop/ruby/combined.yml')
    puts yaml_file
arofhashars=YAML.load_file(yaml_file)
#~ arofhashars.each do |obj|
#~ obj.each do |d| puts d end
#~ end
$fyl.puts "ok check this:"
