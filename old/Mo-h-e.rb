...
class MelodyObject
   ...
   def to_json(lvl=0, spacer="",incchk=false)
     ...
     if nokids then
       ...
       str=spacer+"{id:#{lvl},text:\""+self.name.to_s+"\"}"
       $jfyl.puts str
     else
        case lvl
          when 0
     ...
     def to_jsonold(lvl=0, spacer="",incchk=false)
     kidcheck=@children.empty?
      str=spacer+"{id:#{lvl},"
      str=str+"text:\"" + self.name.to_s + "\"" unless lvl==0
      str=str+ "," unless incchk or lvl==0 or kidcheck
     $fyl.puts str
     tempspacer=spacer
     spacer=spacer+'    '
     unless kidcheck
     inc=0
     $fyl.puts spacer+"item:["
     spacer=spacer+'    '
       @children.each do |kid|
         $lvl+=1
         incchk=false
         incchk=true if @children[@children.length-1]==kid
         incchk=false if @children.length==1
         kid.to_jsonold($lvl,spacer,incchk)
       end
       $fyl.puts spacer+"]"
       $fyl.puts tempspacer+"}"   
       else
